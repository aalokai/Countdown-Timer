...
class CountDown:
    def __init__(self, root):
        ...
        self.window.title('Aalok\'s CountDown Timer')
        self.window.resizable(width=False, height=False)
        self.pause = False
        self.button_frame = Frame(self.window, width=240, height=40)
        self.button_frame.place(x=190, y=150)
        self.time_frame = Frame(self.window, width=480,
                                height=120).place(x=0, y=210)
    ...
    def Get_Time(self):
        ...
        if s == 0 and m == 0 and h == 0:
            messagebox.showwarning(
                'Warning!', 'Please select a right time to set')
        else:
            # Start Button
